...
Any Shiny app is comprised of two crucial code components. An 'UI', and a 'Server' component. When running the app, these two must be called in order to build a properly functioning application. Let's briefly go over what each of these components are actually doing in an app.
...
As you can see, this portion of the code is mainly setting up the visuals of our app, as well as providing the means through which we will collect user input. This input will then be properly collected and manipulated in the server function, which we will look at now: 
